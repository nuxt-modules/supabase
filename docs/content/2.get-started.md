...
The unique Supabase URL that Nuxtjs should use on the server side. Only useful when using nuxt SSR and Supabase in a docker-compose configuration (the browser and the server component of Nuxt have to connect to different urls to reach the supabase server)
