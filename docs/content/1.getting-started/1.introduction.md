...
::tip
You can also use the `NUXT_PUBLIC_` prefix (e.g. `NUXT_PUBLIC_SUPABASE_URL`, `NUXT_PUBLIC_SUPABASE_KEY`) to leverage Nuxt's [runtimeConfig](https://nuxt.com/docs/guide/going-further/runtime-config). This is recommended for production deployments as it allows overriding values at runtime without rebuilding.
::
...
Default: `SUPABASE_URL` env variable, or `NUXT_PUBLIC_SUPABASE_URL` at runtime (ex: <https://example.supabase.co>)

The unique Supabase URL which is supplied when you create a new project in your project dashboard.

### `key`

Default: `SUPABASE_KEY` env variable, or `NUXT_PUBLIC_SUPABASE_KEY` at runtime
...
Default: `SUPABASE_SECRET_KEY` env variable, or `NUXT_SUPABASE_SECRET_KEY` at runtime
...
Default: `SUPABASE_SERVICE_KEY` env variable
