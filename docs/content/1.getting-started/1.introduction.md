...
When disabled, the module will use the default Supabase client from the [@supabase/supabase-js](https://github.com/supabase/supabase-js) library which stores session info in local storage. This is useful in certain cases, such as SPA/CSR-only apps, statically generated sites or mobile apps where cookies might not be available.
...
::tip{to="https://nuxt.com/docs/4.x/api/nuxt-config#srcdir"}
`~/` resolves to your Nuxt source directory: the `app/` directory when present (Nuxt 4 default), otherwise the project root (Nuxt 3 or Nuxt 4 without an `app/` directory).
::
...
::warning
Drop the `app/` prefix if your project doesn't have an `app/` directory. To store the file outside `srcDir` (e.g. at the project root alongside `app/`), set `supabase.types` to the matching path, for example `'~~/types/database.types.ts'`.
::
